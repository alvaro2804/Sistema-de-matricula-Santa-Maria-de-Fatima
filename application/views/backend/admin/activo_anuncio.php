...
<table class="table table-bordered table-striped mb-none" id="render-data" style="width:100%">
...
<tbody>
    <?php
    $count = 1;
    $anuncios = $this->db->get_where('anuncio', array('status' => 1))->result_array();
    foreach ($anuncios as $row):
        ...
        <tr>
            ...
            <td width="110"><?php echo date('d M,Y', $row['crear_timestamp']); ?></td>
            <td width="120">
					...
					<!-- ARCHIVAR ANUNCIO -->
					<a href="<?php echo base_url() ?>index.php?admin/anuncio/archivar_anuncio/<?php echo $row['anuncio_id'] ?>" class="btn btn-primary btn-xs" data-placement="top" data-toggle="tooltip" data-original-title="Archivar Anunicio" >
					...
					<!-- ELIMINAR LINK -->
					<a href="#" class="btn btn-danger btn-xs" data-placement="top" data-toggle="tooltip" data-original-title="Borrar" onclick="confirm_modal('<?php echo base_url(); ?>index.php?admin/anuncio/eliminar/<?php echo $row['anuncio_id']; ?>');">
